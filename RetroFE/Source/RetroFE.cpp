...
bool RetroFE::run( )
{
    ...
    config_.getProperty( "attractModeGameTime", attractModeGameTime );
    ...
    attract_.idleMaxTime        = static_cast<float>(attractModeGameTime);
    ...
    while ( running )
    {
        ...
        if ( running )
        {
		   ...
		   if (attractReturn == 3) // Launch a game in demo mode
		   {
			std::ofstream output("/home/arcade/diamonds/ATTRACT_MODE.txt");
			Logger::write( Logger::ZONE_INFO, "AttractMode", "Time limit reached, state = RETROFE_LAUNCH_ENTER" );
